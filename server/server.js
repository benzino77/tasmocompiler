...
const { listenPort, tasmotaRepo, userPlatformioIni, userConfigOvewrite } = require('./config/config');
...
app.get('/api/v1/repoavailability', (req, res) => {
  isGitRepoAvailable().then((result) => {
...
app.get('/download/firmware.bin', (req, res) => {
  const firmwareFile = path.resolve(tasmotaRepo, '.pioenvs/custom/firmware.bin');
